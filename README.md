...
The **Quarkus Application** template (`templates/quarkus-app`) carries `backstage.io/template-version`, and each scaffolded app sets `spec.scaffoldedFrom: template:default/quarkus-app`. When a platform engineer bumps that version in GitLab `rhdh/rhdh-templates`, Developer Hub opens merge requests on downstream source repos.
